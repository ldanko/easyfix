...
<fix type='FIXT' major='1' minor='1' servicepack='0'>
  ...
  <messages>
    <message msgcat='admin' msgtype='0' name='Heartbeat'>
      <field name='TestReqID' required='N'/>
      <component name='TestComponent' required='N'/>
    </message>
  </messages>
  <components>
    <component name='TestComponent'>
...
fn test_incompatible_versions() {
    ...
    assert_matches!(
        result,
        Err(Error::Builder(BuilderError::IncompatibleVersion))
    );
}
...
fn test_unexpected_message_category_error() {
    ...
    assert_matches!(
        result,
        Err(Error::Validation(ValidationError::UnexpectedMessageCategory(MsgCat::App, name))) if name == "NewOrder"
    )
}
...
fn test_component_flattening() {
    ...
    assert!(
        flattened_heartbeat
            .members()
            .iter()
            .all(|m| matches!(m.definition(), MemberDefinition::Field(_)))
    );
    assert!(flattened_heartbeat.members().iter().all(|m| m.required()));
}
        ...
        match member.definition() {
            MemberDefinition::Field(field) => {
                match field.name.as_str() {
                    ...
                    // SecurityID is optional in InstrumentComponent
                    "SecurityID" => {
                    ...
                    "OrderID" => assert!(
                        member.required(),
                        "OrderID should be required (from its component definition)"
                    ),
                    // Price is optional in OrderComponent
                    "Price" => assert!(!member.required(), "Price should not be required"),

                    // SecurityIDSource is required in SecurityIDComponent.
                    // Flattening preserves the member's own required flag from its
                    // component definition.
                    "SecurityIDSource" => assert!(
                        member.required(),
                        "SecurityIDSource should be required (from its component definition)"
                    ),
...
fn test_field_lookups() {
    ...
    assert_eq!(variants[0].name(), "HEARTBEAT");
...
fn test_message_lookups() {
    ...
    assert_eq!(heartbeat.msg_type(), "0".parse::<MsgType>().unwrap());
...
fn test_raw_data_pairing() {
    // Dictionary with Length+Data pair and standalone Length field
    let raw_data_xml = r#"
        <?xml version='1.0' encoding='UTF-8'?>
        <fix type='FIX' major='4' minor='4' servicepack='0'>
          <header>
          ...
          <fields>
            ...
            <field name='XmlData' number='213' type='XMLDATA'/>
          </fields>
        </fix>
    "#;

    let test_file = TestFile::new("raw_data.xml", raw_data_xml);
    let dictionary = DictionaryBuilder::new()
        .with_fix_xml(test_file.path())
        .build()
        .expect("Failed to build dictionary");

    // BodyLength in header should remain a plain Field (Length not followed by Data)
    let header = dictionary.header();
    let body_length_member = &header.members()[1];
    assert_matches!(
        body_length_member.definition(),
        MemberDefinition::Field(f) if f.name() == "BodyLength" && f.data_type() == BasicType::Length,
        "BodyLength should be a plain Field, not RawData"
    );
    ...
    assert_eq!(
        ...
        2,
        "TestReqID + RawData(EncodedText)"
    );

    let test_req = &heartbeat.members()[0];
    assert_matches!(
        test_req.definition(),
        MemberDefinition::Field(f) if f.name() == "TestReqID"
    );

    let encoded_text = &heartbeat.members()[1];
    assert_matches!(
        encoded_text.definition(),
        MemberDefinition::RawData { length, data }
            if length.name() == "EncodedTextLen" && data.name() == "EncodedText"
                && length.data_type() == BasicType::Length
                && data.data_type() == BasicType::Data,
        "EncodedTextLen + EncodedText should be paired as RawData"
    );
    assert!(!encoded_text.required());
    assert_eq!(encoded_text.name(), "EncodedText");

    // Accessor methods
    let (len_field, data_field) = encoded_text.as_raw_data().unwrap();
    assert_eq!(len_field.number(), 354);
    assert_eq!(data_field.number(), 355);
    assert!(encoded_text.is_raw_data());
    ...
    assert_eq!(
        nos.members().len(),
        3,
        "ClOrdID + MaxMessageSize + RawData(XmlData)"
    );

    let max_msg_size = &nos.members()[1];
    assert_matches!(
        max_msg_size.definition(),
        MemberDefinition::Field(f) if f.name() == "MaxMessageSize" && f.data_type() == BasicType::Length,
        "MaxMessageSize should remain a plain Field (not followed by Data)"
    );

    // XmlDataLen + XmlData should be paired as RawData
    let xml_data = &nos.members()[2];
    assert_matches!(
        xml_data.definition(),
        MemberDefinition::RawData { length, data }
            if length.name() == "XmlDataLen" && data.name() == "XmlData"
                && data.data_type() == BasicType::XmlData,
        "XmlDataLen + XmlData should be paired as RawData"
    );
}

#[test]
fn test_builder_with_strict_check_fix50_empty_header() {
    let fixt11_file = setup_fixt11_file();
    let fix50_file = setup_fix50_file();

    // FIX 5.0+ app dictionaries have empty header/trailer (defined in FIXT).
    // Strict check should accept this without returning UnknownField("BeginString").
    let result = DictionaryBuilder::new()
        .with_fixt_xml(fixt11_file.path())
        .with_fix_xml(fix50_file.path())
        .with_strict_check(true)
    ...
    assert!(
        result.is_ok(),
        "Strict check should pass for FIX 5.0+ with empty header/trailer: {:?}",
        result.err()
    );
}

#[test]
fn test_builder_with_strict_check_fix50_msg_type_not_unused() {
    let fixt11_file = setup_fixt11_file();

    // FIX 5.0+ app dictionaries may define MsgType in their <fields> section
    // even though the header (which uses MsgType) is defined in FIXT.
    // MsgType should not be flagged as unused when the header is empty.
    let fix50_with_msg_type = r#"
        <?xml version='1.0' encoding='UTF-8'?>
        <fix type='FIX' major='5' minor='0' servicepack='0'>
          <header/>
          <trailer/>
    ...
    let fix50_file = TestFile::new("FIX50_msg_type.xml", fix50_with_msg_type);
    ...
    assert!(
        result.is_ok(),
        "MsgType should not be flagged as unused in FIX 5.0+ with empty header: {:?}",
        result.err()
    );
}
