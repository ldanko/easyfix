...
fn test_required_flag_parsing() {
    ...
    for (xml, expected) in xml_variants {
        ...
        assert_matches!(member, Member::Field(r) if r.required == expected, "Required flag mismatch for: {}", xml);
    }
...
fn parse_header() {
    ...
    assert_matches!(begin_string, Member::Field(r) if r.name == "BeginString" && r.required);
    ...
    assert_matches!(no_hops, Member::Group(g) if g.name == "NoHops" && !g.required && g.members.len() == 3);
}
...
fn parse_component() {
    ...
    assert_matches!(
        &component.members[3],
        Member::Group(g) if g.name == "NoSecurityAltID" && !g.required && g.members.len() == 2
    );
}
...
fn parse_group() {
    ...
    assert_matches!(
        &group.members[3],
        Member::Group(g) if g.name == "NoPartySubIDs" && !g.required && g.members.len() == 2
    );
}
...
fn parse_message() {
    ...
    assert_matches!(
        &message.members[0],
        Member::Field(r) if r.name == "EncryptMethod" && r.required
    );

    // Check a component
    assert_matches!(
        &message.members[11],
        Member::Component(r) if r.name == "MsgTypeGrp" && !r.required
    );
}
