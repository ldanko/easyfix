...
use std::{
    borrow::Cow, cell::Cell, io, net::SocketAddr, ops::RangeInclusive, rc::Rc, time::Duration,
};
...
fn serialize_msg(
    admin: AdminBase<'static>,
    msg_seq_num: SeqNum,
    sending_time: UtcTimestamp,
) -> Vec<u8> {
    let mut msg = Message::from_admin(HeaderBase::default(), admin);
    msg.set_sender_comp_id(CLIENT_COMP_ID.to_owned());
    msg.set_target_comp_id(SERVER_COMP_ID.to_owned());
    msg.set_msg_seq_num(msg_seq_num);
    msg.set_sending_time(sending_time);
...
fn test_request(id: &'static FixStr) -> AdminBase<'static> {
    AdminBase::TestRequest(TestRequestBase {
        test_req_id: Cow::Borrowed(id),
    })
}
...
async fn pump_until_logon<S: MessagesStorage + 'static>(acceptor: &mut Acceptor<Message, S>) {
    loop {
...
async fn read_until_pumping<S: MessagesStorage + 'static, R: AsyncRead + Unpin>(
    acceptor: &mut Acceptor<Message, S>,
    reader: &mut R,
    buf: &mut Vec<u8>,
    needle: &[u8],
    count: usize,
...
fn reject_flood_does_not_kill_connection_task() {
    ...
    run_local_test(async {
        let mut acceptor: Acceptor<Message, _> =
            Acceptor::new(settings(), Box::new(|_| InMemoryStorage::new()));
...
fn panicked_connection_task_releases_session() {
    run_local_test(async {
        ...
        let mut acceptor: Acceptor<Message, _> = Acceptor::new(
            settings(),
        ...
        acceptor.disable_with_logout(Some(SessionStatusBase::SessionLogoutComplete.into()), None);
